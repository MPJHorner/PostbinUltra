...
use crate::{
    request::{CapturedRequest, ForwardBody, ForwardOutcome, ForwardStatus},
    store::RequestStore,
};
...
async fn handle(
    ...
    if let Some(forward) = forward_snapshot {
        ...
        .await;
        // Persist the outcome on the captured request so the UI's "Forwarded"
        // tab can render it, before we hand the upstream body back to the
        // original client.
        let _ = state.store.append_forward(captured_id, outcome.clone());
        return response_from_outcome(&outcome, captured_id);
    }
...
/// Executes a forward attempt and returns a fully-described outcome. Pure of
/// axum response concerns so that both the live capture handler and the
/// desktop "Replay" action share the same machinery.
#[allow(clippy::too_many_arguments)]
pub async fn do_forward(
    ...
    if body_truncated {
        return ForwardOutcome {
            ...
            status: ForwardStatus::Skipped {
                reason: "captured body was truncated (exceeded max body size); forwarding skipped to avoid corrupting the upstream view".into(),
            },
        };
    }
    ...
    let response = match upstream {
        Ok(r) => r,
        Err(e) => {
            tracing::warn!(error = %e, "forward upstream failed");
            return ForwardOutcome {
                started_at,
                upstream_url,
    ...
    let mut out_headers: Vec<(String, String)> = Vec::with_capacity(response.headers().len());
    for (name, value) in response.headers().iter() {
        let v = String::from_utf8_lossy(value.as_bytes()).into_owned();
        out_headers.push((name.as_str().to_string(), v));
    }

    // Buffer the upstream body so we can both store it on the outcome and
    // return it to the original client. Postbin is a dev/inspection tool —
    // small bodies are the norm, and the outcome would be useless otherwise.
    ...
    let body = ForwardBody::from_bytes(&body_bytes);
...
/// Build an axum response from a forward outcome — used by the live capture
/// handler to relay the upstream reply back to the original client.
fn response_from_outcome(outcome: &ForwardOutcome, captured_id: Uuid) -> Response {
    match &outcome.status {
        ...
        ForwardStatus::Skipped { reason } => {
            forward_error_response("forward_skipped_truncated_body", reason, captured_id)
        }
        ForwardStatus::Error { message, .. } => {
            forward_error_response("forward_failed", message, captured_id)
        }
    }
}
